...
struct AnyContextCommandHandler {
    ...
    init<Handler: ContextCommandHandling>(_ handler: Handler) {
        ...
        invocationClosure = { command in
            guard let decoded = command.decode(as: Handler.Command.self) else {
                return nil
            }
...
struct ContextCommandHandlers {
    ...
    func invocation(
        ...
        handlers[command.featureID]?.invocation(for: command)
    }
}
...
final class ContextCommandRouter {
    ...
    func prepare(
        ...
        guard let invocation = handlers.invocation(for: command) else {
            logger.error(
                "No valid Handler for context command \(command.featureID.rawValue, privacy: .public)"
            )
            return nil
        }
    ...
    /// 执行已经成功恢复的调用，并管理其本地异步任务生命周期。
    /// - Parameters:
    ///   - invocation: `prepare` 产生的类型化调用。
    ///   - requestID: 主应用为反馈、进度和任务索引生成的本地标识。
    /// - Returns: Task 已经登记时为 `true`。
    @discardableResult
    func run(
        _ invocation: ContextCommandInvocation,
        requestID: UUID
    ) -> Bool {
        guard inFlightTasks[requestID] == nil else {
            logger.error(
                "Refused to replace in-flight task for request \(requestID.uuidString, privacy: .public)"
            )
            return false
        }
