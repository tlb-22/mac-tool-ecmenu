...
@MainActor
final class MenuConfigurationController: NSObject, ObservableObject {
    /// 状态页观察的当前有效配置。
    ...
    private static func storedConfiguration(
        ...
        return MenuConfigurationChannel.decodedConfiguration(from: data)
    }

    /// 编码当前配置并写入主应用偏好存储。
    private func storeConfiguration() {
        guard let data = MenuConfigurationChannel.encodedData(
            for: configuration
        ) else {
            logger.error("Could not encode the menu configuration for storage")
            return
        }
        defaults.set(
            data,
