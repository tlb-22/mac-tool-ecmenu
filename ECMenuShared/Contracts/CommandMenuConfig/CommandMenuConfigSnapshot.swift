/**
 把菜单开关与有序模板描述组合为可跨进程整体应用的快照，区分有效空库和模板不可用。
...
nonisolated struct CommandMenuConfigSnapshot: Codable, Equatable, Sendable {
    static let currentSchemaVersion = 1
    static let standard = CommandMenuConfigSnapshot(
    ...
    init(configuration: CommandMenuConfig, newFileTemplates: [FileTemplateMenuItem]) {
        self.init(configuration: configuration, fileTemplateState: .available(newFileTemplates))
    }

    init(configuration: CommandMenuConfig, fileTemplateState: FileTemplateMenuState) {
    ...
    init(from decoder: Decoder) throws {
        ...
        configuration = try container.decode(CommandMenuConfig.self, forKey: .configuration)
