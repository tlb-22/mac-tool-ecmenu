...
nonisolated struct CommandMenuConfig: Codable, Equatable, Sendable {
    /// 当前持久化和传输格式版本；领域状态本身不保存该值。
    static let currentSchemaVersion = CommandMenuConfigEnvelope.currentSchemaVersion

    /// 所有功能使用产品默认可见性时的配置。
    static let standard = CommandMenuConfig()
    ...
    init(from decoder: Decoder) throws {
        self = try CommandMenuConfigEnvelope(from: decoder).configuration
    }

    /// 把领域状态包装为当前版本信封。
    func encode(to encoder: Encoder) throws {
        try CommandMenuConfigEnvelope(configuration: self).encode(to: encoder)
    }
}

/// 持久化与传输专用的版本信封；解码后只向业务层交付有效配置。
nonisolated private struct CommandMenuConfigEnvelope: Codable, Sendable {
    /// 当前持久化和传输格式版本。
    ...
    /// 使用当前领域状态创建待编码信封。
    init(configuration: CommandMenuConfig) {
