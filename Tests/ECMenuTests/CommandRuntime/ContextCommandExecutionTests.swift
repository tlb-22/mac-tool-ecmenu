/**
 验证命令 Router 的准备、异步执行与取消生命周期，以及进度状态的独立所有权。
 注入 Handler 和渲染回调，覆盖延迟显示、多任务、取消及关闭动作的路由。
 */
...
final class ContextCommandExecutionTests: XCTestCase {
    ...
    /// 延迟显示、迟到取消和未开始任务的清理保持幂等。
    func testProgressStateIgnoresLateLifecycleEvents() {
        let requestID = UUID()
        var state = ContextCommandProgressState()
    ...
    func testProgressCenterDoesNotRenderFastCompletedTask() async {
        ...
        let center = ContextCommandProgressCenter(
            displayDelay: .zero,
            render: { items, _ in renderedSnapshots.append(items) }
        )
    ...
    /// 呈现边界收到的动作必须回到同一个状态所有者，关闭界面不撤回任务取消意图。
    func testRenderedActionsRouteCancellationAndDismissalToProgressOwner() async throws {
        let requestID = UUID()
        var renderedItems: [ContextCommandProgressItem] = []
        var renderedActions: ContextCommandProgressActions?
        let center = ContextCommandProgressCenter(displayDelay: .zero) { items, actions in
            renderedItems = items
            renderedActions = actions
        }
        center.begin(
            requestID: requestID,
            descriptor: progressDescriptor(id: "rendered-actions", title: "任务"),
            totalUnitCount: 2
        )
        await waitUntil { renderedItems.map(\.requestID) == [requestID] }
        let actions = try XCTUnwrap(renderedActions)

        actions.cancel(requestID)

        XCTAssertTrue(center.isCancellationRequested(for: requestID))
        XCTAssertEqual(renderedItems.first?.isCancellationRequested, true)
        actions.dismiss()
    ...
    func testRouterPreparesThenRunsRegisteredTypeErasedHandler() async throws {
        ...
        let router = ContextCommandRouter(handlers: handlers, progressCenter: ContextCommandProgressCenter(render: { _, _ in }))
        ...
        XCTAssertNotNil(handler.presentedRequestID)
    }
    ...
    func testRouterRejectsUnknownAndUndecodableEnvelopes() throws {
        ...
        let router = ContextCommandRouter(
            ...
            progressCenter: ContextCommandProgressCenter(render: { _, _ in })
        )
