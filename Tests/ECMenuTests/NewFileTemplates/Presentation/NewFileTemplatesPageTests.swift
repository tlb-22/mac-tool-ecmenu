...
@MainActor
final class NewFileTemplatesPageTests: XCTestCase {
    func testUnchangedNamesSwitchBothDirectionsWithoutReplacingNativeFields() async throws {
        let fixture = try NewFileTemplatesPageFixture()
    ...
    private func operationButtons(in host: NewFileTemplatesNativePageHost) throws -> [NewFileTemplatesNativeAccessibilityElement] {
        host.layout()
        let buttons = accessibilityElements(in: host.view).filter { $0.role == .button }
        return try [NewFileTemplatesText.open, NewFileTemplatesText.replace, NewFileTemplatesText.delete, NewFileTemplatesText.add].map { title in
...
@MainActor
private struct NewFileTemplatesPageFixture {
