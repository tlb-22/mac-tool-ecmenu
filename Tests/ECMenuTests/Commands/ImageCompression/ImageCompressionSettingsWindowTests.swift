/**
 验证真实压缩参数窗口与异步请求的一次性完成和独立会话关系。
 使用隔离偏好检查确认保存顺序、关闭与任务取消，并检查双语校验文案布局。
 */
...
final class ImageCompressionSettingsWindowTests: XCTestCase {
    ...
    /// 两个窗口连续确认时，偏好在回调中立即更新，不依赖等待任务的恢复顺序。
    func testConcurrentConfirmationsPersistBeforeWaitingTasksResume() async throws {
        let fixture = try makeSettingsStore()
        defer { fixture.cleanup() }
        var controllers: [ImageCompressionSettingsWindowController] = []
        let prompt = ImageCompressionSettingsPrompt { controllers.append($0) }
        let coordinator = ImageCompressionSettingsCoordinator(store: fixture.store) { settings, onConfirm in
            await prompt.request(settings: settings, onConfirm: onConfirm)
        }
        let first = Task { await coordinator.requestSettings() }
        for _ in 0..<100 where controllers.count < 1 { await Task.yield() }
        let second = Task { await coordinator.requestSettings() }
        for _ in 0..<100 where controllers.count < 2 { await Task.yield() }
        defer {
            first.cancel()
            second.cancel()
        ...
        try maximumWidthField(in: controllers[0]).stringValue = "800"
        try maximumWidthField(in: controllers[1]).stringValue = "1600"
        try compressButton(in: controllers[1]).performClick(nil)
        XCTAssertEqual(fixture.store.load().maximumWidth, 1600)
        try compressButton(in: controllers[0]).performClick(nil)
        XCTAssertEqual(fixture.store.load().maximumWidth, 800)
        let firstResult = await first.value
        let secondResult = await second.value
        XCTAssertEqual(firstResult?.maximumWidth, 800)
        XCTAssertEqual(secondResult?.maximumWidth, 1600)
        XCTAssertEqual(fixture.store.load().maximumWidth, 800)
    }
    ...
    func testAlreadyCancelledTaskDoesNotPresentWindow() async throws {
        ...
        let task = Task { @MainActor in
            ...
            let prompt = ImageCompressionSettingsPrompt { controller in
                run.controller = controller
            }
            let coordinator = ImageCompressionSettingsCoordinator(store: fixture.store) { settings, onConfirm in
                await prompt.request(settings: settings, onConfirm: onConfirm)
            }
            run.result = await coordinator.requestSettings()
            run.didReturn = true
            completed.fulfill()
        }
        defer { task.cancel() }
    ...
    private func startPrompt(
        ...
        let task = Task { @MainActor in
            let prompt = ImageCompressionSettingsPrompt { controller in
                run.controller = controller
                if presentWindow { controller.present() }
            }
            let coordinator = ImageCompressionSettingsCoordinator(store: settingsStore) { settings, onConfirm in
                await prompt.request(settings: settings, onConfirm: onConfirm)
            }
            run.result = await coordinator.requestSettings()
            run.didReturn = true
