/**
 验证隐藏和显示操作的点号名称规则、失败分类与批量结果保留。
 在隔离文件系统检查属性写入只作用于选中对象，且符号链接不改变目标。
 */
...
final class VisibilityTests: XCTestCase {
    ...
    func testFileSystemExecutionIsNonRecursiveAndDoesNotFollowSymlinks() async throws {
        ...
        let hideReport = await HideItemsHandler(platform: .system, present: { _, _, _ in }).execute(
            HideItemsCommand(
                selection: selection([
                    ordinary,
                    directory,
    ...
    func testFailureKeepsSuccessfulItemsAndContinuesBatch() {
        let first = url("/test/first")
        let failed = url("/test/failed")
        let last = url("/test/last")
        let report = VisibilityExecution.execute(
            VisibilityRules.makePlan(operation: .hide, itemURLs: [first, failed, last]),
            platform: VisibilityPlatform(setHidden: { item, hidden in
                XCTAssertTrue(hidden)
                if item == failed { throw POSIXError(.EACCES) }
            })
        )
        XCTAssertEqual(report.succeededCount, 2)
        XCTAssertEqual(report.issues.map(\.itemURL), [failed])
        XCTAssertEqual(report.issues.first?.kind, .permissionDenied)
    }
