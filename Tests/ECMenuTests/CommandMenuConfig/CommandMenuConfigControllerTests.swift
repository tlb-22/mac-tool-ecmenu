...
@MainActor
final class CommandMenuConfigControllerTests: XCTestCase {
    func testChangesPersistBeforePublicationAndNoOpDoesNotPublish() throws {
        let suite = "CommandMenuConfigTests.\(UUID().uuidString)"
        let defaults = try XCTUnwrap(UserDefaults(suiteName: suite))
        defer { defaults.removePersistentDomain(forName: suite) }
        let store = CommandMenuConfigStore(defaults: defaults)
        var published: [CommandMenuConfig] = []
        let controller = CommandMenuConfigController(store: store, publisher: MenuChangePublisher {
        ...
        let restored = CommandMenuConfigController(store: store, publisher: MenuChangePublisher {
