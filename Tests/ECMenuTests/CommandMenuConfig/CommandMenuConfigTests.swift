...
final class CommandMenuConfigTests: XCTestCase {
    ...
    func testVisibilityUsesOnlyHiddenFeatureIDs() throws {
        let featureID = CreateNewFileCommand.descriptor.id
        var configuration = CommandMenuConfig.standard
    ...
    func testCurrentFormatRoundTrip() throws {
        ...
        let data = CommandMenuConfigChannel.encodedData(for: configuration)
        let decoded = try CommandMenuConfigChannel.decodedConfiguration(
            from: data
        )
        ...
        XCTAssertEqual(
            ...
            CommandMenuConfig.currentSchemaVersion
        )
    }
    ...
    func testUnsupportedSchemaIsRejected() throws {
        ...
        XCTAssertThrowsError(
            try CommandMenuConfigChannel.decodedConfiguration(from: data)
        )
    }
    ...
    func testIncompleteCurrentSchemaIsRejected() throws {
        let data = try JSONSerialization.data(withJSONObject: [
            "schemaVersion": CommandMenuConfig.currentSchemaVersion,
            "hiddenFeatureIDs": [],
        ])
