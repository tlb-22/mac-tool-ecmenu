...
final class NewTextFileTests: XCTestCase {
    ...
    func testAsynchronousExecution() async throws {
        ...
        guard case .failure(let failure) = await handler.execute(
            missingCommand
        ) else {
            return XCTFail("A missing directory must fail")
        }
        XCTAssertEqual(failure.directoryURL, missingDirectory)
        XCTAssertEqual(failure.kind, .unavailable)
        XCTAssertFalse(failure.systemError.domain.isEmpty)
    }
