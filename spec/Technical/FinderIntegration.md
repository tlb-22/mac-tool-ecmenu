...
每个实际渲染的菜单叶子都在本次快照上完成可用性判断，并把已经构造好的功能命令绑定为独立调用。后续菜单请求不得覆盖仍在显示的旧菜单调用；当前实现用菜单项实例唯一的 `tag` 路由并消费该绑定。这用于消除 action 到达前又发生菜单请求时的覆盖风险，不能退化为“最新一次 Finder 状态”的单一可变缓存，也不能在点击时重新解释另一份 Finder 状态。
...
用户配置、冻结的 Finder 快照和 Feature 在本次构建读取的系统事实共同决定叶子能否准备出类型化命令。无法准备命令的叶子在创建 AppKit 菜单前即被删除；同一棵递归菜单树支持子菜单和分隔线，解析时删除空子菜单以及每一层开头、结尾和连续的分隔线。所有实际渲染的叶子都绑定已经准备完成的调用和唯一 tag。
