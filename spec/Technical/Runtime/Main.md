...
各专题以职责模块定位实现，表格分别列出核心类型和源码入口。标注“组合”的节点或表项汇总多个协作模块；模块内部的判断、读取和写入步骤在所属流程中展开。紧密相关的类型可以共处一个文件，同一类型也可以按职责分布在多个文件，具体归属遵循[目录组织规则](../Architecture/DirectoryRules.md)。
