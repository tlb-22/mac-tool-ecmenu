...
- **Finder Extension**解释 Finder 事件、构建菜单、冻结语义上下文并投递命令。它保持 App Sandbox，不执行产品文件操作，也不呈现业务结果。
...
Finder Extension 对每次菜单点击构造一份不可变命令请求，建立一条定向连接并只发送一次。命令通道是 one-way send：Extension 不等待主应用返回接管状态或业务执行结果，也不设置自动重试、交付超时、请求去重或提交状态查询。
...
主应用未运行或瞬时通信失败时，本次点击结束，用户可在宿主恢复后再次点击。用户的每次点击是一个独立请求；产品不尝试在进程之间提供 exactly-once 交付。Finder 上下文等编码形状发生不兼容变化时必须提升请求 schema，不依赖宽松解码猜测旧负载。
...
进度报告和用户取消是两个独立能力。窗口中的取消只传递协作意图，由具体命令在不会破坏单项输出的边界响应；它不替代 Router 对应用生命周期使用的 Swift Task cancellation。每个功能自行定义计数单位、安全取消边界和部分结果语义，通用执行层只负责让任务结束时同步结束对应的进度生命周期。
...
主应用关闭 App Sandbox，Finder Extension 保持 App Sandbox。动作链路不弹出逐目录 `NSOpenPanel`，也不保存 security-scoped bookmark；Extension 只采集菜单上下文，文件访问由主应用完成。
