...
final class CommandMenuConfigReplica: NSObject {
    private enum RefreshState {
        case idle
        case fetching(refreshAgain: Bool)
    }
    /// 当前用于构建 Finder 菜单的配置快照。
    private var configuration: CommandMenuConfigSnapshot

    /// 在主应用不可用时恢复最后有效快照的本地偏好存储。
    private let cache: CommandMenuConfigCacheStore

    /// 只接受精确主应用签名响应的定向 socket 客户端。
    private let transport: (any CommandMenuConfigRequesting)?
    ...
    init(
        ...
        transport: (any CommandMenuConfigRequesting)?
    ) {
        let cache = CommandMenuConfigCacheStore(defaults: defaults)
        self.cache = cache
        ...
        DistributedNotificationCenter.default().addObserver(
            ...
            name: CommandMenuConfigChannel.didChangeNotification,
            object: nil
        )
    ...
    func refreshConfiguration() {
        ...
        transport.fetchCommandMenuConfig { [self] result in
    ...
    private func applyRefreshResult(
        _ result: Result<CommandMenuConfigSnapshot, Error>
    ) {
